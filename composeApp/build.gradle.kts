...
fun publishWindowsMsiOutput(release: Boolean) {
    ...
    val packageArtifactName = "Apachiy"
    val finalMsi = outputDir.resolve("$packageArtifactName-Windows-$windowsPlayerBridgeArch-$desktopReleaseVersionName.msi")
    val defaultMsiCandidates = listOf(
        outputDir.resolve("$packageArtifactName-$desktopReleasePackageVersion.msi"),
        outputDir.resolve("Nuvio-$desktopReleasePackageVersion.msi"),
    )
    val sourceMsi = defaultMsiCandidates.firstOrNull { it.exists() }
        ?: finalMsi.takeIf { it.exists() }
        ?: outputDir.listFiles { file -> file.isFile && file.extension.equals("msi", ignoreCase = true) }
